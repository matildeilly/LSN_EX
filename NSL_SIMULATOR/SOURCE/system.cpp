...
{ // Perform a simulation step
  ...
  // cout<<double(_naccepted)/_nattempts<<endl;
  return;
}
...
{ // Propose a MC move for particle i
  ...
  { // Gibbs sampler for Ising
    double deltaE = 2 * _J * (_particle(this->pbc(i - 1)).getspin() + _particle(this->pbc(i + 1)).getspin()) + 2 * _H;
    double prob = 1. / (1 + exp(-_beta * deltaE));
    if (_rnd.Rannyu() < prob)
    {
...
{ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while (!input.eof())
  {
    ...
    else if (property == "GOAL_TEMP")
    {
      input >> _goal_temp;
      coutf << "GOAL_TEMPERATURE= " << _goal_temp << endl;
    }
  ...
  if (input.is_open())
  {
    while (!input.eof())
    {
      ...
      else if (property == "PRESSURE")
      {
        ...
        index_property++;
        _ptail = 32 * M_PI * _rho * (1.0 / (9.0 * pow(_r_cut, 9)) - 1.0 / (6.0 * pow(_r_cut, 3)));
      }
...
// * * EQUILIBRIUM: è stata usata in tutte le esercitazioni per monitorare la grandezza di interesse al fine di capire dopo quanti steps la simulazione è in equilibrio.
{ // Measure properties
  /*
    ...
    double T=0.0; //temperature
      ...
      for (int i=0; i<_npart; i++) {K += 0.5 * dot( _particle(i).getvelocity() , _particle(i).getvelocity() );}
      K /= double(_npart);
  ...
  */
  ...
  for (int i = 0; i < _npart - 1; i++)
  {
    for (int j = i + 1; j < _npart; j++)
    {
      ...
      if (dr < _r_cut)
      {
        U += 1.0 / pow(dr, 12) - 1.0 / pow(dr, 6); // POTENTIAL ENERGY
      }
    }
  }

  U = _vtail + 4.0 * U / double(_npart);
  return U;
  // * * ESERCITAZIONE 7
}
...
{ // Measure properties
  ...
  double pressure_temp = 0.0; // temporary accumulator for pressure  //M.A.
  double magnetization = 0.0;
  double virial = 0.0;
  vec gofr_temp;
  gofr_temp.set_size(_n_bins);
  gofr_temp.zeros();
  if (_measure_penergy or _measure_pressure or _measure_gofr)
  {
    for (int i = 0; i < _npart - 1; i++)
    {
      for (int j = i + 1; j < _npart; j++)
      {
        ...
        distance(2) = this->pbc(_particle(i).getposition(2, true) - _particle(j).getposition(2, true), 2);
        dr = sqrt(dot(distance, distance));
        // GOFR // * * ESERCITAZIONE 7
        for (int k = 0; k < _n_bins; k++)
        {
          if (dr >= k * _bin_size && dr <= (k + 1) * _bin_size)
          {
            gofr_temp(k) += 2;
          }
        }
        if (dr < _r_cut)
        {
          ...
          // PRESSURE //* * ESERCITAZIONE 4
          if (_measure_pressure)
            pressure_temp += 1.0 / pow(dr, 12) - 1.0 / (2 * pow(dr, 6)); // M.A.
        }
      }
    }
  }
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
  if (_measure_pressure) //** ESERCITAZIONE 4 
  {                                                                                                                                                     
    pressure_temp = _ptail + _rho * /*_measurement(_index_temp) [ESERCITAZIONE 4]*/ _temp /*[ESERCITAZIONE 7]*/ + 48.0 / (3 * _volume) * pressure_temp; 
  ...
  if (_measure_cv)
  {
    ...
    for (int i = 0; i < _npart; i++)
    {
      s_i = double(_particle(i).getspin());
      s_j = double(_particle(this->pbc(i + 1)).getspin());
      h += -_J * s_i * s_j - 0.5 * _H * (s_i + s_j);  //è la formula dell'Hamiltoniana
    }
    _measurement(_index_cv) = h * h;
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi)
  {
    ...
    _measurement(_index_chi) = _beta * pow(sum_s_i, 2) / double(_npart);
  }
  // GOFR ////////////////////////////////////////////////////////////////
  if (_measure_gofr)
  {
    ...
    {
      double deltaV = 4.0 / 3 * M_PI * (pow(i * _bin_size + _bin_size, 3) - pow(i * _bin_size, 3)); //bin_size è dr e i*bin_size è r
      _measurement(_index_gofr + i) = gofr_temp(i) / (_rho * _npart * deltaV); // il numero di conteggi su ciasun bin è visto come una proprietà a sè del sistema (ogni bin ha un suo index...) --> ciò rende il codice più semplice
    }
  }
...
void System ::averages(int blk)
{
  ...
  _average(_index_cv) = _beta * _beta * (_average(_index_cv) - pow(_average(_index_tenergy) * _npart, 2)) / double(_npart); //** ESERCITAZIONE 6: finisco il calcolo della heat capacity  
  _global_av += _average;
